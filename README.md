...
如果某个辅助库或可选数据库（例如 `session/session.db`、`message/media_0.db`、`head_image/head_image.db`、`message/message_revoke.db`、`favorite/favorite.db`、`sns/sns.db`）当前没有在微信进程内找到 key，`init` 会输出 warning 并继续保留其他已准备好的 key。联系人和数字消息分片 key 仍然是必需项，缺失时需要重新运行 `sudo wxview init`。
...
`~/.wxview/cache/<account>/mtime.json`，源 DB 的 size、mtime 和 salt 没变且 cache 已存在时会跳过解密。daemon 日志写入 `~/.wxview/wxview.log`。
...
- 如果缺少对应 message DB 的 key，命令会提示先运行 `sudo wxview init`，不会在查询路径临时扫描 key。
...
查询微信收藏本地缓存 `favorite/favorite.db`。这个库是可选数据面；如果当前账号没有拿到对应 key，先运行 `sudo wxview init`，如果仍缺失则等微信打开过收藏相关数据后再重试。
...
- 可选消息索引库：未来可以增加 `wxview index build/status/refresh`，物化一张本地消息表，
  并为 `chat_username + create_time/seq`、`create_time + chat_username` 建普通索引。
  这会同时加速 `messages` 和 `timeline`。
- 可选 FTS5 正文索引：现有 `wxview search` 先走本地扫描，后续可以在同一个索引库中增加 FTS5 表。
...
- 特别感谢 [wechat-decrypt](https://github.com/ylytdeng/wechat-decrypt)：这个项目提供了 macOS 微信数据库解密方向上的关键参考，是 Wxview 能够落地本地解密与缓存流程的重要基础。
